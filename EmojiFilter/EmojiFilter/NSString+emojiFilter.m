...
bool isEmojiSetContainsChar(long singleChar) {
    ...
    while (low <= high) {
        ...
        mid = (low + high)/2;
        //printf("%ld\n",emoji_set[mid]);
        if (emoji_set[mid] == singleChar) {
            return true;
        } else if (emoji_set[mid] < singleChar) {
            low = mid + 1;
        } else {
            high = mid - 1;
        }
...
- (BOOL)containsEmoji {
    ...
    [self enumerateSubstringsInRange:NSMakeRange(0, self.length)
                            ...
                            usingBlock:^(NSString * _Nullable substring, NSRange substringRange, NSRange enclosingRange, BOOL * _Nonnull stop) {
                                ...
                                NSInteger charLength =  substring.length;
                                for (int i = 0; i<charLength; i++) {
                                    long c = [substring characterAtIndex:i];
                                    long realChar = c;
                                    if (c >=0xD800 && c < 0xDBFF && ++i < charLength) {
                                        int nextChar = [substring characterAtIndex:i];
                                        realChar =  (0x10000 + (c - 0xD800) * 0x400 + (nextChar - 0xDC00));
                                    }
